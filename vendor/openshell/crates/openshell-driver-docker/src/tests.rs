...
#[test]
fn build_container_create_body_maps_configured_fuse_device() {
    let mut config = runtime_config();
    config.fuse_device = Some(PathBuf::from("/dev/fuse"));

    let create_body = build_container_create_body(&test_sandbox(), &config).unwrap();
    let device = create_body
        .host_config
        .as_ref()
        .and_then(|host_config| host_config.devices.as_ref())
        .and_then(|devices| devices.first())
        .expect("FUSE device should be mapped");
