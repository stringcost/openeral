...
use bollard::models::{
    ContainerCreateBody, ContainerSummary, ContainerSummaryStateEnum, DeviceMapping, DeviceRequest,
...
pub struct DockerComputeConfig {
    ...
    /// Optional host FUSE device to expose to sandbox containers.
    ///
    /// The path is interpreted by the Docker daemon, not by the gateway
    /// process. This allows the gateway itself to run in a container with only
    /// Docker socket access while the host daemon maps `/dev/fuse` into the
    /// sandbox containers it creates.
    pub fuse_device: Option<PathBuf>,
    ...
    pub async fn new(
        ...
        let driver = Self {
            ...
            config: DockerDriverRuntimeConfig {
                ...
                supervisor_bin,
                guest_tls,
                fuse_device: docker_config.fuse_device.clone(),
...
fn docker_fuse_devices(config: &DockerDriverRuntimeConfig) -> Option<Vec<DeviceMapping>> {
    config.fuse_device.as_ref().map(|path| {
...
fn build_container_create_body(
    ...
    Ok(ContainerCreateBody {
        ...
        host_config: Some(HostConfig {
            ...
            devices: docker_fuse_devices(config),
