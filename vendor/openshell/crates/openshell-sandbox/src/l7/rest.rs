...
use crate::secrets::{
    PreparedHttpRequest, ScopedSecretInjector, contains_placeholder_bytes,
    ...
    U: AsyncRead + AsyncWrite + Unpin,
{
    ...
    let prepared = PreparedHttpRequest {
        bytes: rewrite_result.rewritten,
        swaps: Vec::new(),
    };

    relay_http_request_with_prepared_request_guarded(
        req,
        client,
        upstream,
        &prepared,
...
pub(crate) async fn relay_http_request_with_prepared_request_guarded<C, U>(
    req: &L7Request,
    client: &mut C,
    upstream: &mut U,
